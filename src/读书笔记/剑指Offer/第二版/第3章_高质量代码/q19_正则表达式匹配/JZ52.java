package 读书笔记.剑指Offer.第二版.第3章_高质量代码.q19_正则表达式匹配;
            ...
            for (int j = 1; j < n + 1; j++) {
                ...
                if (pattern.charAt(j - 1) == '*') {
                    if (dp[i][j - 2]) {// p[j-2]出现0次
                        ...
                        dp[i][j] = true;
                    } else if (dp[i - 1][j] && pattern.charAt(j - 2) == '.') {// 最特殊情况:p[j-2]=. p[j-1]=*时 根据条件知是万能匹配
                        dp[i][j] = true;
                    }
                } else {// 当p[j-1]!=*时,有两种情况
                    if (dp[i - 1][j - 1] && str.charAt(i - 1) == pattern.charAt(j - 1)) {// 前面元素之前都匹配 且 当前元素也相同
                        dp[i][j] = true;
